...
Resolution order for each DB_* setting (first wins):
  ...
  2. scripts/labor_safety/.env.script  (gitignored, user-maintained)
...
LS_ROOT = Path(__file__).resolve().parents[1]            # scripts/labor_safety/
REPO_ROOT = LS_ROOT.parents[1]                            # repo root
...
_ENV_SCRIPT = _read_env(LS_ROOT / ".env.script")
